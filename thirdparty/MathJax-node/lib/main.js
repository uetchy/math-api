...
//
var MathJaxPath = 'https://math.now.sh/mathjax/MathJax.js'
...
function ConfigureMathJax() {
  window.MathJax = {
    ...
    AuthorInit: function() {
      ...
      MathJax.Ajax.loaded[
        MathJax.Ajax.fileURL('[MathJax]/extensions/MathEvents.js')
      ] = true
      ...
      MathJax.Hub.Register.MessageHook('SVG Jax - unknown char', function(
        message
      ) {
        AddError(
          'SVG - Unknown character: U+' +
            message[1].toString(16).toUpperCase() +
            ' in ' +
            (message[2].fonts || ['unknown']).join(','),
          !undefinedChar
        )
      })
      MathJax.Hub.Register.MessageHook(
        'CommonHTML Jax - unknown char',
        function(message) {
          AddError(
            'CHTML - Unknown character: U+' +
              message[1].toString(16).toUpperCase() +
              ' in ' +
              (message[2].fonts || ['unknown']).join(','),
            !undefinedChar
      ...
      MathJax.Hub.Register.MessageHook('MathML Jax - parse error', function(
        message
      ) {
        AddError('MathML - ' + message[1])
      })
      MathJax.Hub.Register.MessageHook('AsciiMath Jax - parse error', function(
        message
      ) {
        ...
        SVG.BBOX.TEXT.Augment({
          Init: function(scale, text, def) {
            ...
            var textWidth = text
              .split('')
              .map(function(c) {
                return isFullwidthCodePoint(c.codePointAt())
                  ? data.cjkCharWidth
                  : 8.5
              })
              .reduce(function(a, b) {
            ...
            this.element.setAttribute(
              'transform',
              'scale(' + scale + ') matrix(1 0 0 -1 0 0)'
            )
            ...
            toSVG: function(variant, scale) {
              ...
              if (values.src !== '' && !MGLYPH.GLYPH[values.src]) {
                if (!values.width || !values.height) {
                  AddError(
                    'mglyphs must have explicit width and height in mathjax-node'
                  )
                }
      ...
      MathJax.Hub.Register.StartupHook('CommonHTML Jax Config', function() {
        ...
        CHTML.Augment({
          ...
          getHDW: function(c, name, styles) {
            return { h: 0.8, d: 0.2, w: c.length * 0.5 }
          },
        })
      ...
      //
      if (
        MathJax.AuthorConfig.extensions.indexOf('TeX/color.js') == -1 &&
        MathJax.AuthorConfig.extensions.indexOf('TeX/autoload-all.js') == -1
      ) {
      ...
      MathJax.Hub.Register.StartupHook('End', function() {
        if (MathJax.OutputJax.SVG.resetGlyphs)
  ...
  if (MathJaxConfig) {
    ...
    if (MathJaxConfig.extensions) {
      window.MathJax.extensions = window.MathJax.extensions.concat(
        MathJaxConfig.extensions
      )
    }
  }
}
...
function GetHTML(result) {
  ...
  if (data.speakText) {
    ...
    for (child of labelTarget.childNodes)
...
function StartQueue() {
  ...
  SVG.config.linebreaks.automatic = CHTML.config.linebreaks.automatic =
    data.linebreaks
  SVG.config.linebreaks.width = CHTML.config.linebreaks.width =
    data.width * data.ex
  ...
  //
  var renderer =
    data.html || data.htmlNode || data.css
      ? 'CommonHTML'
      : data.svg || data.svgNode
      ? 'SVG'
      : 'None'
